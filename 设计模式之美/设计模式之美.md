...
![image-20210526172035404](设计模式之美.assets/image-20210526172035404.png)
...
- 对于复杂的数据结构（比如树、图）来说，有各种**复杂的遍历方式**---------树有前中后序、按层遍历，图有深度优先、广度优先遍历等等。针对图的遍历，我们就可以定义 DFSIterator、BFSIterator 两个迭代器类，分别来实现深度优先遍历和广度优先遍历
...
![image-20210526172448900](设计模式之美.assets/image-20210526172448900.png)

## 在遍历同时增删元素

![image-20210526180008854](设计模式之美.assets/image-20210526180008854.png)

![image-20210526180019473](设计模式之美.assets/image-20210526180019473.png)



删除游标后面的元素（元素 c 和 d），这样就不会存在任何问题了

![image-20210526180118495](设计模式之美.assets/image-20210526180118495.png)

![image-20210526180137727](设计模式之美.assets/image-20210526180137727.png)

## 如何应对

- 一种是遍历的时候不允许增删元素，（难--确定遍历开始和结束的时间点）并不一定非要把所有元素都遍历一遍，可以定义一个新的接口 finishIteration()，但是也增加了开发成本，还很容易漏掉。
...
在 ArrayList 中定义一个成员变量modCount，记录集合被修改的次数

创建迭代器的时候，我们把modCount 值传递给迭代器的 expectedModCount 成员变量，每次调用迭代器上的 hasNext()、next()、currentItem() 函数，我们都会检查集合上的 modCount 是否等于expectedModCount，也就是看，在创建完迭代器之后，modCount 是否改变过

![image-20210602150704318](设计模式之美.assets/image-20210602150704318.png)

## 如何在遍历的同时安全地删除集合元素？

还定义了一个`remove()` 方法，能够在遍历集合的同时，安全地删除集合中的元素

它只能删除游标指向的前一个元素，而且一个 next() 函数之后，只能跟着最多一个 remove() 操作，多次调用 remove() 操作会报错。

![image-20210602152448874](设计模式之美.assets/image-20210602152448874.png)

![image-20210602152516098](设计模式之美.assets/image-20210602152516098.png)

通过迭代器去删除这个元素的时候，我们可以更新迭代器中的游标和 lastRet 值，来保证不会因为删除元素而导致某个元素遍历不到。

#  中介模式

中介模式**定义了一个单独的（中介）对象**，来封装一组对象之间的**交互**。将这组对象之间的交互委派给与中介对象交互，来避免对象之间的直接交互

![image-20210602153335524](设计模式之美.assets/image-20210602153335524.png) 
